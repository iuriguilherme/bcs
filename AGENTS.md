...
1. **Source Files** (`src/` directory, `index.css`, `dev.html`) - Individual source files  
2. **Bundled File** (`index.html`) - **AUTO-GENERATED** single file with all CSS/JS inlined

> [!CAUTION]
> **NEVER EDIT `index.html` DIRECTLY!** It is auto-generated by the build script and will be overwritten. Always edit source files (`src/*.js`, `index.css`, `dev.html`) and then rebuild.

> [!IMPORTANT]  
> **The build process**: `dev.html` (body) + `index.css` (styles) + `src/*.js` (scripts) → `index.html`
