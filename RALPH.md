...
1. Use the workflows:brainstorm command from the "compound-engineering" plugin before starting anything. This will generate a brainstorm document in docs/brainstorms/. Use that document for the next step;  
2. Use the workflows:plan command from the "compound-engineering" plugin using the brainstorm document as argument. This will generate a plan document in docs/plans/. Use that document for the next step, but before going to the next step, run the command deepen-plan to update the document;  
3. Use the workflows:work command from the "compound-engineering" plugin using the plan document as argument. Use the same command more times until the plan document is implemented;  
4. Use the workflows:review command from the "compound-engineering" plugin after the work session. Use the same command more times until all work has been reviewed and problems identified;  
5. Use the workflows:compound command from the "compound-engineering" plugin after the review is finished. Ensure all knowledge is documented, files README.md, CLAUDE.md, GEMINI.md, AGENTS.md and RALPH.md are all updated during this step.  
...
The files CLAUDE.md and GEMINI.md are similar to the file AGENTS.md however they are tailored for specific agents. CLAUDE.md is for Claude Code which is the tool we are using, and GEMINI.md is for Gemini Cli which we also use eventually. The three files should be generally consistent, albeit some differences related to the tool stack they belong to may be present.  
